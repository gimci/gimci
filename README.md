...
### buildNew
json 형태로 데이터를 생성한다. Romanize된 데이터를 가지고 하기때문에 한글로된 파일은 convertFileHangyrToRoman을 통해 로마자로 된 파일로 우선 변형하여야 한다. json의 정보는 txt정보를 가지고 delete연산을 통해 변화가능한 모든 정보를 hashtable형태로 저장한다. 이 hashtable은 key 와 elem으로 구성되고 key는 모든 변형가능한 데이터이고 elem에는 refer정보 즉, delete가 되기전의 정보를 저장하고 있다.
